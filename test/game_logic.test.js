...
// ─── Security regressions: the server owns all money and phase invariants ──

test("截拍防作弊：不能提交不存在的钞票，拒绝后状态与资金不变", () => {
  const game = makeSnipeState({ highestBid: 100 });
  const aliceBefore = structuredClone(game.players[0].money);
  const bobBefore = structuredClone(game.players[1].money);
  const cardBefore = game.auctionCard;

  const result = game.actionSnipe(true, { 500: 2 }); // Alice 没有 500 面值

  assert.ok(result.error, "伪造钞票应返回 error");
  assert.deepStrictEqual(game.players[0].money, aliceBefore, "付款人资金不应变化");
  assert.deepStrictEqual(game.players[1].money, bobBefore, "收款人资金不应变化");
  assert.strictEqual(game.auctionCard, cardBefore, "牌仍应处于截拍阶段");
  assert.strictEqual(game.phase, "SNIPE");
});

test("钞票载荷校验：拒绝未知面值、负数、小数与数组", () => {
  const cases = [
    { 999: 1 },
    { 100: -1 },
    { 100: 0.5 },
  ...
  for (const paid of cases) {
    const game = makeSnipeState({ highestBid: 100 });
    const result = game.actionSnipe(true, paid);
    assert.ok(result.error, `应拒绝 ${JSON.stringify(paid)}`);
    assert.strictEqual(game.phase, "SNIPE");
  }
});

test("私盘防作弊：报价不能超过实际持有，拒绝后不能标记为已提交", () => {
  const game = new GameState(["Alice", "Bob"]);
  game.phase = "AUCTION";
  game.currentPlayerIndex = 0;
  const mk = (n) => ({ cardId: `stamps_0${n}`, cardName: "x", flavorText: "", setId: "stamps", setName: "绝版邮票", setScore: 90 });
  game.players[0].antiques = [mk(1)];
  game.players[1].antiques = [mk(2)];
  game.players[0].money = { 0: 2, 10: 0, 50: 0, 100: 1, 200: 0, 500: 0 };

  assert.ok(game.startPrivateDeal(0, 1, "stamps").event);
  const result = game.submitDealOffer(0, { 100: 2 });

  assert.ok(result.error, "透支报价应被拒绝");
  assert.strictEqual(game.dealInitiatorSubmitted, false);
  assert.deepStrictEqual(game.dealOffer, {});
});

test("私盘入口校验：拒绝错误阶段、自己作为目标、无共同套系", () => {
  const game = new GameState(["Alice", "Bob"]);

  assert.ok(game.startPrivateDeal(0, 1, "stamps").error, "WAITING 阶段不能私盘");

  game.phase = "AUCTION";
  assert.ok(game.startPrivateDeal(0, 0, "stamps").error, "不能与自己私盘");
  assert.ok(game.startPrivateDeal(0, 1, "not_a_set").error, "未知套系应拒绝");
  assert.ok(game.startPrivateDeal(0, 1, "stamps").error, "双方没有共同套系应拒绝");
});

test("拍卖输入校验：拒绝字符串、小数、负数出价", () => {
  const game = new GameState(["A", "B"]);
  game.phase = "AUCTION";
  game.deck = [
    { cardId: "stamps_01", cardName: "x", flavorText: "", setId: "stamps", setName: "绝版邮票", setScore: 90 },
  ];
  game.startAuction();
  const bidder = game.bidTurnId;

  for (const amount of ["100", 10.5, -10, 0]) {
    const result = game.placeBid(bidder, amount);
    assert.ok(result.error, `应拒绝非法出价 ${amount}`);
    assert.strictEqual(game.highestBid, 0);
  }
});
